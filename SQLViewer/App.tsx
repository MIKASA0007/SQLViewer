...
import { Alert, AppState, AppStateStatus, NativeModules, Platform, Linking, DeviceEventEmitter } from 'react-native';
...
function App(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const fileOpenListener = DeviceEventEmitter.addListener('onOpenFile', (event) => {
  ...
  const handleAppStateChange = useCallback((nextAppState: AppStateStatus) => {
    if (
      ...
      console.log('DEBUG: App state changed to active, checking for pending intents');
      checkPendingFileUri();
    }
  ...
  const processIncomingFile = useCallback(async (uri: string, providedFileName?: string): Promise<void> => {
    if (isProcessingFile) {
    ...
    try {
      ...
      const fileName = providedFileName || uri.split('/').pop() || 'unknown.sql';
